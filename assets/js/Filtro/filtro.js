    ...
    $(document).on('click', '.guardar-cambios', function(){
        ...
        $.post('filtro/actualizar_descripcion', {id_incidencia,descripcion}, function(response){
            ...
            if (id_incidencia === undefined) {
                ...
                $('.div-texto-mensaje').css({'background-color':'#E52141'});
                $('.texto-mensaje').html("Tiene que seleccionar algún reporte");
                $('.img-mensaje').attr('src',$('.img-mensaje').attr('incorrecto'));
                $('.img-mensaje').css({'width':'30%'});
                $('.mensaje').css({'visibility':'visible'});
                $('.contenedor-mensaje-filtro').css({'box-shadow':'0 0px 45px 0 rgba(179, 34, 34, 0.349'});
                $('.contenedor-mensaje-filtro').css({'width':'450px'});
                $('.contenedor-mensaje-filtro').css({'height':'239px'});
                $('.contenedor-mensaje-filtro').css({'transform':'translateY(0%)'});
            }else {
                //Se abre el modal con la imagen correcta
                $('.texto-mensaje').css({'color':'#fff'});
                $('.div-texto-mensaje').css({'background-color':'#55BA7D'});
                $('.texto-mensaje').html(json.msg);
                $('.img-mensaje').attr('src',$('.img-mensaje').attr('correcto'));
                $('.img-mensaje').css({'width':'35%'});
                $('.mensaje').css({'visibility':'visible'});
                $('.contenedor-mensaje-filtro').css({'box-shadow':'0 0px 50px 0 rgba(31, 175, 26, 0.349)'});
                $('.contenedor-mensaje-filtro').css({'width':'450px'});
                $('.contenedor-mensaje-filtro').css({'height':'227px'});
                $('.contenedor-mensaje-filtro').css({'transform':'translateY(0%)'});
            }
            url = json.url;
        }); 
